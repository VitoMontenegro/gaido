...
func NewRouter(cfg config.Config, log *slog.Logger, h *handlers.Handlers) http.Handler {
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Group(func(pr chi.Router) {
			...
			pr.Get("/account/transport/rides", h.ListMyTransportRides)
			pr.Post("/account/transport/rides", h.CreateTransportRide)
			pr.Put("/account/transport/rides/{id}", h.UpdateTransportRide)
			pr.Delete("/account/transport/rides/{id}", h.DeleteTransportRide)
			pr.Post("/account/transport/bookings", h.CreateTransportBooking)
			pr.Get("/account/transport/bookings", h.ListMyTransportBookings)
			pr.Get("/account/transport/bookings/incoming", h.ListIncomingTransportBookings)
			pr.Get("/account/transport/bookings/{id}", h.GetTransportBooking)
			pr.Patch("/account/transport/bookings/{id}/status", h.UpdateTransportBookingStatus)
			pr.Get("/account/carrier/billing/plans", h.ListCarrierBillingPlans)
			pr.Get("/account/carrier/billing/status", h.GetCarrierBillingStatus)
			pr.Post("/account/carrier/billing/checkout", h.CarrierBillingCheckout)
			pr.Post("/account/carrier/billing/confirm/{id}", h.CarrierBillingConfirm)
			pr.Get("/account/carrier", h.GetCarrierAccount)
			pr.Put("/account/carrier/profile", h.UpsertCarrierAccount)
			pr.Post("/account/carrier/vehicles", h.CreateCarrierVehicle)
			pr.Put("/account/carrier/vehicles/{id}", h.UpdateCarrierVehicle)
			pr.Delete("/account/carrier/vehicles/{id}", h.DeleteCarrierVehicle)
		})
		...
		api.Group(func(mr chi.Router) {
			...
			mr.Get("/moderator/transport/rides", h.ModListTransportRides)
			mr.Post("/moderator/transport/rides/{id}/approve", h.ModApproveTransportRide)
			mr.Post("/moderator/transport/rides/{id}/reject", h.ModRejectTransportRide)
			mr.Get("/moderator/carriers", h.ModListCarriers)
			mr.Post("/moderator/carriers/{id}/approve", h.ModApproveCarrier)
		...
		api.Group(func(ar chi.Router) {
			...
			ar.Get("/admin/carriers", h.AdminListCarriers)
			ar.Put("/admin/carriers/{id}", h.AdminUpdateCarrier)
