...
func (h *Handlers) updateProviderStatus(w http.ResponseWriter, r *http.Request, action string) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || !domain.ValidProviderStatus(req.Status) {
		response.Error(w, r, apperrors.ErrValidation)
		return
	}
	...
	_ = h.Audit.Log(r.Context(), &actor, action, "provider", &providerID, "", req.Status, r.RemoteAddr, r.UserAgent())
