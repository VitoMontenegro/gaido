...
import OfferingCard from '../components/discover/OfferingCard'
...
function countByCategory(items: DiscoverOffering[]) {
  ...
  for (const item of items) {
    map.set(item.category_name, (map.get(item.category_name) ?? 0) + 1)
  }
        ...
        <header className="space-y-4">
          ...
          {!loc.hasLocation && (
            <div className="card p-4">
              <p className="mb-3 font-medium">📍 Де ви шукаєте?</p>
              <LocationPicker />
            </div>
          )}
          {loc.hasLocation && (
                ...
                {stats.categories.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {stats.categories.map(([name, count]) => (
                      <span key={name} className="rounded-full bg-sand-100 px-3 py-1 text-xs text-ink">
                        {name} · {count}
                      </span>
                    ))}
                  </div>
            ...
            <div className="grid gap-6">
              {view === 'map' && (
                <div className="space-y-4">
                  ...
                  <DiscoverMap points={localMapPoints} center={anchor} />
                  {items.length > 0 && (
                    <div className="space-y-3">
                      <h2 className="text-sm font-medium text-ink">Усі пропозиції ({items.length})</h2>
                      <div className="grid gap-4 sm:grid-cols-2">
                        {items.map((item: DiscoverOffering) => (
                          <OfferingCard key={item.id} item={item} compact />
                        ))}
                      </div>
                    </div>
                  )}
              ...
              {view === 'list' && (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {!isLoading && !useSectionFallback && items.length === 0 && (
                    <p className="text-muted">
                      {loc.hasLocation
                        ? 'Пропозицій у цій зоні поки немає. Спробуйте збільшити радіус або обрати інше місто (наприклад, Berlin).'
                        : 'Пропозицій у цьому розділі поки немає.'}
                    </p>
                  )}
                  {!isLoading && useSectionFallback && items.length === 0 && (
                    <p className="text-muted">Пропозицій у цьому розділі поки немає.</p>
                  )}
                </div>
              )}
            </div>
